...
authorUsername = message.author.username
  ...
  db.prepare(`UPDATE eco SET coins = ? WHERE id = ?`).run(result.coins - args[1], message.author.id);
  if (result2 === undefined) {
    db.prepare(`INSERT INTO eco (id, coins) VALUES(?, ?);`).run(message.mentions.users.first().id, parseInt(args[1]));
    } else {
